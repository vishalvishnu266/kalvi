...
use crate::shared::{LoginForm, password, db};
...
async fn authenticate_user(
    ...
    let session = session_manager.create_session(user.id, ip_address, user_agent)
        .await
...
fn render_login_page(error: Option<String>, redirect: Option<String>) -> Markup {
    html! {
        (DOCTYPE)
        html lang="en" {
            head {
...
async fn show_login_page(Query(query): Query<LoginQuery>) -> impl IntoResponse {
    ...
    Html(render_login_page(error, query.redirect))
}
...
async fn process_login(
    ...
    match authenticate_user(&pool, form.login.clone(), ip_address, user_agent).await {
        ...
        Err(error) => {
            Html(render_login_page(Some(error), form.redirect)).into_response()
        }
    }
}
