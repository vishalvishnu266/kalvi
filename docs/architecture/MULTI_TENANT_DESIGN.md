...
Each tenant gets its own SQLite database file named `tenant_{slug}.db`. This gives us:
...
Path convention: `/t/{slug}/...`. `shared::tenant_middleware` extracts the slug, looks up the tenant, and injects the tenant `SqlitePool` into request extensions.
...
3. Calls `db_manager.tenant_pool(&database_name)` which:
   - Creates `tenant_{slug}.db` on disk (`.create_if_missing(true)`).
   - Runs `TENANT_MIGRATOR` against it (idempotent via `_sqlx_migrations`).
4. Calls `auth::create_admin_user(&pool, username, password)` with the credentials from the form to seed the initial admin.
