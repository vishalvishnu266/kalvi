...
// The folder where `npm run bundle:ota` drops zipped React builds.
// Kept relative to this crate's directory so `cargo run` works out-of-the-box.
...
// Use a DIFFERENT port than the Vue app so both OTA servers can run side by
// side while comparing the two implementations.
...
async fn main() {
    ...
    println!("🚀 Axum OTA Server (React) listening on http://0.0.0.0:{}", port);
