...
export function useCamera() {
    ...
    const checkPermission = useCallback(async () => {
        try {
            ...
            const next = (p as any)?.camera ?? 'unknown';
            setPermission(next);
            return next;
        } catch (e: any) {
    ...
    const takePhoto = useCallback(async () => {
        ...
        try {
            ...
            if (e?.message?.toLowerCase?.().includes('cancel')) setError(null);
            else setError(e?.message || 'Camera failed');
            return null;
        } finally {
            setBusy(false);
        }
    ...
    const pickPhoto = useCallback(async () => {
        ...
        try {
            ...
            if (e?.message?.toLowerCase?.().includes('cancel')) setError(null);
            else setError(e?.message || 'Photo picker failed');
            return null;
        } finally {
            setBusy(false);
        }
