...
use crate::dto::auth_dto::LoginForm;
use crate::exception::web_error::{render, WebError};
use crate::security::session_user::{
    read_cookie_from_headers, COOKIE_SESSION, COOKIE_TENANT, COOKIE_USER,
};
...
async fn post_login_with_redirect(
    ...
    match auth_service::login(&pool, &f.identifier, &f.password).await {
        Ok(user) => {
            let session =
                match auth_service::issue_session(&state.sessions, user.id, None, None).await {
                    Ok(s) => s,
                    Err(e) => {
                        return render(&LoginPage {
    ...
    if let (Some(tid), Some(token)) = (cookie_tenant, cookie_session) {
        if let Ok(t) = validate_tenant_id(tid) {
            if let Ok(_pool) = state.pool_for(&t).await {
                let _ = auth_service::revoke_session(&state.sessions, &token).await;
            }
        }
    }
