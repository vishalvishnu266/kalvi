//! Portal (parent/student self-service) global web handlers.
...
use crate::dto::portal_dto::{LinkTenantForm, PortalLoginForm, PortalRegisterForm};
use crate::entity::portal_membership::NewPortalMembership;
use crate::entity::portal_user::NewPortalUser;
use crate::entity::user::User;
use crate::exception::web_error::{render, WebError};
use crate::security::session_user::read_cookie_from_headers;
use crate::service::{auth_service, portal_service};
...
pub async fn post_login(
    ...
    let Some(user) =
        portal_service::find_portal_user_by_identifier(&state.system, &f.identifier).await?
    else {
...
pub async fn post_register(
    ...
    let user = portal_service::create_portal_user(
        &state.system,
        &NewPortalUser {
...
pub async fn home(State(state): State<AppState>, headers: HeaderMap) -> Result<Response, WebError> {
    ...
    let user = portal_service::get_portal_user(&state.system, uid)
        .await
...
pub async fn post_link_tenant(
    ...
    let user: User = auth_service::login(&pool, &f.identifier, &f.password)
        .await
        .map_err(|_| WebError::forbidden("invalid tenant credentials"))?;
    let roles = auth_service::roles_of(&pool, user.id).await?;
    ...
    portal_service::add_portal_membership(
        &state.system,
        &NewPortalMembership {
            portal_user_id,
...
async fn build_membership_views(
    ...
    let memberships = portal_service::list_portal_memberships(&state.system, portal_user_id).await?;
    let mut out = Vec::new();
