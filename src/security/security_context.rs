...
impl SecurityContext {
    ...
    pub fn require(&self, perm: &str) -> ServiceResult<()> {
        if self.has_permission(perm) {
            Ok(())
        } else {
            Err(ServiceError::forbidden(perm))
        }
    }

    pub fn require_any(&self, perms: &[&str]) -> ServiceResult<()> {
        if matches!(self.actor, Actor::System { .. })
            || perms.iter().any(|p| self.has_permission(p))
        {
            Ok(())
        } else {
            Err(ServiceError::forbidden(&perms.join(" | ")))
        }
    }
}
