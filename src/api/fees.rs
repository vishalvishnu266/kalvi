...
{
    scope.ctx.require_any(&[perm::FEES_VIEW, perm::FEES_COLLECT])?;
...
{
    scope.ctx.require(perm::FEES_COLLECT)?;
    ...
    -> Result<Json<FeeInvoice>, ServiceHttpError>
{
    Ok(Json(scope.services.fees.generate_invoice_for_student(
        &scope.ctx, b.student_id, b.fee_structure_id, b.invoice_no,
...
{
    scope.ctx.require_any(&[perm::FEES_VIEW, perm::FEES_VIEW_OWN, perm::FEES_COLLECT])?;
...
{ scope.services.fees.cancel_invoice(&scope.ctx, id).await?; Ok(StatusCode::NO_CONTENT) }

pub async fn outstanding(scope: TenantScope, Path((_t, sid)): Path<(String, i64)>)
    -> Result<Json<serde_json::Value>, ServiceHttpError>
{
    let v = scope.services.fees.outstanding(&scope.ctx, sid).await?;
...
pub async fn overdue(scope: TenantScope, Query(q): Query<Today>)
    -> Result<Json<Vec<FeeInvoice>>, ServiceHttpError>
{
    scope.ctx.require(perm::FEES_VIEW)?;
...
pub async fn aging(scope: TenantScope, Query(q): Query<Today>)
    -> Result<Json<serde_json::Value>, ServiceHttpError>
{
    let (b0, b30, b60, b90) = scope.services.fees.aging(&scope.ctx, q.today).await?;
...
{ Ok(Json(scope.services.fees.record_payment(&scope.ctx, b).await?)) }
