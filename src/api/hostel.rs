...
{ Ok(Json(scope.services.hostel.allocate(b.student_id, b.hostel_room_id, b.from_date).await?)) }
...
pub async fn transfer(scope: TenantScope, Json(b): Json<Transfer>)
    -> Result<Json<HostelAllocation>, ServiceHttpError>
{ Ok(Json(scope.services.hostel.transfer_room(b.student_id, b.to_room_id, b.from_date).await?)) }
...
{ scope.services.hostel.vacate(sid, b.to_date).await?; Ok(StatusCode::NO_CONTENT) }
