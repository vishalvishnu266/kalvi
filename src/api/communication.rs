//! `/api/{tenant}/communication/*` handlers.
...
{
    let (ann, pushed) = scope.services.communication.broadcast(b).await?;
...
{ scope.services.repos.announcements.delete(id).await?; Ok(StatusCode::NO_CONTENT) }
