...
{ Ok(Json(scope.services.inventory.low_stock_alert_ids().await?)) }
...
{ scope.services.inventory.set_po_status(id, &b.target).await?; Ok(StatusCode::NO_CONTENT) }
