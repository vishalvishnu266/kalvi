...
{
    let id = scope.services.payroll.set_salary(b.staff_id, b.effective_from, b.items).await?;
...
{ Ok(Json(scope.services.payroll.generate_payslip(b.staff_id, b.month, b.year).await?)) }
...
pub async fn generate_month(scope: TenantScope, Json(b): Json<GenerateMonth>)
    -> Result<Json<serde_json::Value>, ServiceHttpError>
{
    let n = scope.services.payroll.generate_month(b.month, b.year).await?;
...
{ scope.services.payroll.approve(id).await?; Ok(StatusCode::NO_CONTENT) }
...
pub async fn pay(scope: TenantScope, Path((_t, id)): Path<(String, i64)>, Json(b): Json<PayBody>)
    -> Result<StatusCode, ServiceHttpError>
{ scope.services.payroll.pay(id, b.paid_on, b.from_bank).await?; Ok(StatusCode::NO_CONTENT) }
