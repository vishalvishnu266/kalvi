...
use crate::models::academic::{
    AcademicYear, Grade, NewAcademicYear, NewRoom, NewSubject, NewTerm, Room, Section, Subject,
    Term,
};
use crate::services::academic as ac_svc;
use crate::services::perm;

pub async fn list_years(scope: TenantScope) -> Result<Json<Vec<AcademicYear>>, ServiceHttpError> {
...
pub async fn list_terms(
    ...
    Ok(Json(
        ac_svc::list_terms(&scope.pool, &scope.ctx, year_id).await?,
    ))
}
...
pub async fn list_grades(scope: TenantScope) -> Result<Json<Vec<Grade>>, ServiceHttpError> {
    scope.ctx.require(perm::ACADEMIC_VIEW)?;
    Ok(Json(ac_svc::list_grades(&scope.pool).await?))
}

pub async fn list_sections(scope: TenantScope) -> Result<Json<Vec<Section>>, ServiceHttpError> {
    scope.ctx.require(perm::ACADEMIC_VIEW)?;
    Ok(Json(ac_svc::list_sections(&scope.pool).await?))
}

pub async fn list_rooms(scope: TenantScope) -> Result<Json<Vec<Room>>, ServiceHttpError> {
    scope.ctx.require(perm::ACADEMIC_VIEW)?;
...
pub async fn list_subjects(scope: TenantScope) -> Result<Json<Vec<Subject>>, ServiceHttpError> {
    scope.ctx.require(perm::ACADEMIC_VIEW)?;
