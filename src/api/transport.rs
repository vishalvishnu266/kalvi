...
{ Ok(Json(scope.services.transport.assign_to_stop(b.student_id, b.route_stop_id, b.valid_from).await?)) }
...
{ scope.services.transport.end_assignment(id, b.on).await?; Ok(StatusCode::NO_CONTENT) }
