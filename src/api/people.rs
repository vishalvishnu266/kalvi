...
{
    Ok(Json(scope.services.people.list_students_for(&scope.ctx, p.limit, p.offset).await?))
}
...
pub async fn search_students(scope: TenantScope, Query(q): Query<SearchQ>)
    -> Result<Json<Vec<Student>>, ServiceHttpError>
{
    scope.ctx.require_any(&[perm::STUDENTS_VIEW, perm::STUDENTS_VIEW_OWN])?;
...
{ Ok(Json(scope.services.people.admit(&scope.ctx, b).await?)) }

pub async fn withdraw(scope: TenantScope, Path((_t, id)): Path<(String, i64)>)
    -> Result<StatusCode, ServiceHttpError>
{ scope.services.people.withdraw(&scope.ctx, id).await?; Ok(StatusCode::NO_CONTENT) }

pub async fn graduate(scope: TenantScope, Path((_t, id)): Path<(String, i64)>)
    -> Result<StatusCode, ServiceHttpError>
{ scope.services.people.graduate(&scope.ctx, id).await?; Ok(StatusCode::NO_CONTENT) }
...
{ Ok(Json(scope.services.people.hire_staff(&scope.ctx, b).await?)) }
...
{
    scope.services.people.terminate_staff(&scope.ctx, id, b.on).await?;
    Ok(StatusCode::NO_CONTENT)
}
