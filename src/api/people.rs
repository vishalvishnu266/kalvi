...
#[derive(Deserialize)]
pub struct Page {
    #[serde(default = "d50")] pub limit: i64,
...
{
    Ok(Json(people_svc::list_students_for(&scope.pool, &scope.ctx, p.limit, p.offset).await?))
}

#[derive(Deserialize)]
pub struct SearchQ { pub q: String, #[serde(default = "d50")] pub limit: i64 }
    ...
    -> Result<Json<Vec<Student>>, ServiceHttpError>
{
    ...
    Ok(Json(people_svc::search_students(&scope.pool, &q.q, q.limit).await?))
}
    ...
    -> Result<Json<Vec<Staff>>, ServiceHttpError>
{
    ...
    Ok(Json(people_svc::list_staff(&scope.pool, p.limit, p.offset).await?))
}

pub async fn hire(scope: TenantScope, Json(b): Json<NewStaff>)
    -> Result<Json<Staff>, ServiceHttpError>
{
    Ok(Json(people_svc::hire_staff(&scope.pool, &scope.ctx, b).await?))
}
...
#[derive(Deserialize)]
pub struct Terminate { on: chrono::NaiveDate }
