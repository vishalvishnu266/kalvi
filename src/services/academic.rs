...
pub async fn list_grades(pool: &SqlitePool) -> RepoResult<Vec<Grade>> {
    Ok(
        sqlx::query_as::<_, Grade>("SELECT * FROM grade ORDER BY level")
            .fetch_all(pool)
            .await?,
    )
}
...
pub async fn list_sections(pool: &SqlitePool) -> RepoResult<Vec<Section>> {
    Ok(
        sqlx::query_as::<_, Section>("SELECT * FROM section ORDER BY name")
            .fetch_all(pool)
            .await?,
    )
}
...
pub async fn list_rooms(pool: &SqlitePool) -> RepoResult<Vec<Room>> {
    Ok(
        sqlx::query_as::<_, Room>("SELECT * FROM room ORDER BY name")
            .fetch_all(pool)
            .await?,
    )
}
...
pub async fn list_subjects(pool: &SqlitePool) -> RepoResult<Vec<Subject>> {
    Ok(
        sqlx::query_as::<_, Subject>("SELECT * FROM subject ORDER BY code")
            .fetch_all(pool)
            .await?,
    )
}
