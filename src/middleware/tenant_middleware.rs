...
pub async fn tenant_middleware(
    ...
    // Path is /web/slug/... or /api/slug/...
    if let (Some(&prefix), Some(&slug)) = (segments.get(0), segments.get(1)) {
        if prefix == "web" || prefix == "api" {
            ...
            // Handle tenant root redirect (/web/{slug} -> dashboard/login)
